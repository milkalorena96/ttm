    ...
    <div id="layoutSidenav_content">
        <main class="container-fluid">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="float-left">
                                <span class="card-title">Nombre: <strong>{{ $restaurante->nombre }}</strong></span>
                            </div>
                            <a href="{{route('restaurante.index')}}" class="btn btn-danger ml-auto float-right">Cerrar</a>
                        </div>

                        <div class="card-body">
                            <div class="form-group">
                                <strong>Nombre: </strong>{{$restaurante->nombre}}&nbsp;|
                                <strong>Ubicación: </strong>{{$restaurante->ubicacion}}
                            </div>
                            <div class="form-group">
                                <strong>Departamento: </strong>{{$restaurante->departamento}}&nbsp;|
                                <strong>Provincia: </strong>{{$restaurante->provincia}}&nbsp;|
                                <strong>Distrito: </strong>{{$restaurante->distrito}}
                            </div>
                            <div class="form-group">
                                <strong>Descripcion:</strong>
                                <textarea class="form-control" rows="2" readonly>{{ $restaurante->descripcion }}</textarea>
                            </div>
                        </div>
                        <div class="card text-center">
                            <p>Subir fotos para el Restuarante <strong>{{ $restaurante->nombre }}</strong> </p>
                            <div class="container-fluid">
                                <form enctype="multipart/form-data" method="post" action="{{route('AgregarFotosRest')}}">
            ...
            <div class="row">
                <div class="col-md-12 ">
                    <div class="card text-center">
                    <p>Fotos de: <strong>{{ $restaurante->nombre }}</strong> </p>
                        <div class="card-body">
                            <section id="exemple" class="container">
                                <div class="wrap">
                                    ...
                                    @foreach($restaurante->fotos as $foto)
                                        <li class="box">
                                            <img src="/img/restaurante/fotos/{{$foto->ruta}}" alt="Placeholder image">

                                            <div class="input-group">
                                                {!!Form::open(['method'=>'DELETE','route'=>['eliminarFotoRest',$foto->id],'class'=>'formEliminar' ])!!}
                                                {!! Form::submit('Eliminar',['class'=>'btn btn-sm btn-danger']) !!}
                                                <a href="/img/restaurante/fotos/{{$foto->ruta}}" target="_blank" class="btn btn-sm btn-primary">Ver</a>
                                                {!! Form::close() !!}
                                            </div>
                                        </li>
                                        @endforeach
